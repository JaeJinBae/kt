...
<script type="text/javascript">
	$(function(){
        ...
        $("#searchBtn").click(function(){
    		...
    		location.href="adminEvent${pageMaker.makeQuery(1)}&searchType="+searchType+"&keyword="+keyword;
    	});
...
<body>
	...
	<div class="contentWrap">
		<div class="leftMenu">
			<h2>게시판 리스트</h2>
			<ul> 
				<li> <a href="${pageContext.request.contextPath}/admin/"> 시공사례</a></li>
				<li> <a href="${pageContext.request.contextPath}/admin/adminEvent" style="font-weight:bold;"> 이벤트</a></li>
			</ul>
		</div>
		<div class="centerMenu"> 
			<h1 class="boardTitle">&lt;이벤트 관리&gt;</h1>
			<div class="tbl_board">
				<table>
					<tr class="tbl_header">
						<th>번호</th>
					...
					<c:choose>
					    ...
					    <c:otherwise>
					        <c:forEach var="item" items="${list}">
								<tr>
									<td>${item.bno}</td>
									<td class="title"><a href="adminEventRead${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}">${item.title}</a></td>
									<td>${item.writer}</td>
									<td><fmt:formatDate type="date" value="${item.regdate}"/></td>
									<td>${item.cnt}</td>
									<td>
										<a href="${pageContext.request.contextPath}/admin/adminEventUpdate${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}"><button>수정</button></a>
										<a href="${pageContext.request.contextPath}/admin/adminEventDelete${pageMaker.makeSearch(pageMaker.cri.page)}&bno=${item.bno}"><button>삭제</button></a>
									</td>
								</tr>	
							</c:forEach>
					    </c:otherwise> 
				...
				<p class="registerBtn"><a href="adminEventRegister${pageMaker.makeSearch(pageMaker.cri.page)}"><button>글쓰기</button></a></p>
				<div class="page">
					<ul>
